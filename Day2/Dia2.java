...
public class Dia2 {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        while (input.hasNextLine()) {// comprueba texto entero
            ...
            int maxgreen=0;
            int maxred=0;
            while (start < linchar.length - 1) {// comprueba linea entera palabra a palabra
                ...
                if (colour == 'b') {
                    if(maxblue<numberofcube[0]){
                        maxblue = numberofcube[0];
                    }
                } else if (colour == 'g') {
                    if(maxgreen<numberofcube[0]){
                        maxgreen = numberofcube[0];
                    }
                } else if (colour == 'r') {
                    if(maxred<numberofcube[0]){
                        maxred = numberofcube[0];
                    }
                }
                start = nextNumber(start, linchar);
            }
            System.out.println("At id "+ id + " maxgreen was "+ maxgreen + " maxblue was "+ maxblue + " and maxred was " + maxred);
            res = (maxgreen*maxblue*maxred) + res;
        }
        input.close();
        System.out.println("The sum of pow is " + res+ '\n');
    }
